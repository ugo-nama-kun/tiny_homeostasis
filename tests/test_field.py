...
from tiny_homeostasis.envs import FieldEnv


def test_make_env():
	env = FieldEnv()
	assert isinstance(env, FieldEnv)
...
def test_action_space():
	...
	assert np.all(env.action_space.shape == (2,))


def test_obs_space():
	env = FieldEnv(dim_resource=3)
...
def test_setting_setpoint():
	env = FieldEnv(internal_reset="setpoint")
...
def test_reset_internal_random_limit(setting, expected_mean, expected_var):
	env = FieldEnv(dim_resource=2, internal_reset="random", internal_random_range=setting)
